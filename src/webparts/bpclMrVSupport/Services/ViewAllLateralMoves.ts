...
export interface ILaterMovesItem {
    Id: number;
    Title: string;
    DateReleased: string;
    FileUrl: string;
...
export default class ViewAllLateralMoves {
    ...
    public async getLateralMoves(): Promise<ILaterMovesItem[]> {

        const items = await this.sp.web.lists
            .getByTitle("MR_SL_LateralMoves")
            .items
        ...
        return items.map((item: any) => {
            ...
            if (item.AttachmentFiles && item.AttachmentFiles.length > 0) {
                ...
                fileUrl = `${this.siteUrl}/Lists/MR_SL_LateralMoves/Attachments/${item.Id}/${fileName}`;
            }
